...
//			return func(args);
    ...
    BootstrapTable.DEFAULTS = {
        ...
        dataName:'rows',//指定数据源表格数据对象名称
        totalName:'total',//指定数据源表格数据对象总行数
        pIndexName:'pageIndex',//指定传入的分页标示
        pSizeName:'pageSize',//指定传入的每页数量
        pageNumber: 1,
        pageSize: 10,
    ...
    BootstrapTable.LOCALES['en-US'] = BootstrapTable.LOCALES['en'] = {
        ...
        formatShowingRows: function (pageFrom, pageTo, totalRows) {
            return sprintf('当前 %s ~ %s行，共 %s行', pageFrom, pageTo, totalRows);
        },
    ...
    BootstrapTable.prototype.initContainer = function () {
        ...
        this.$container.after('<div class="blank2"></div>');
    ...
    BootstrapTable.prototype.initServer = function (silent, query) {
        ...
        if (this.options.queryParamsType === 'limit') {
            ...
            if (this.options.pagination) {
                params[this.options.pIndexName] = this.options.pageNumber - 1;
                params[this.options.pSizeName] = this.options.pageSize;
            }
        }
...
//		debugger;
        data = calculateObjectValue(this.options, this.options.queryParams, {}, data);
        if(data == undefined){
        		data = {};
        }
        $.extend(data, query || params ||{});
...
//          		alert(that.options.responseHandler);
				var _res = res;
				if(that.options.responseHandler){
					_res = that.options.responseHandler(res);
				}
//              res = calculateObjectValue(that.options, that.options.responseHandler, res, res);
                that.load(_res);
                that.trigger('load-success', _res);
            },
            error: function (res) {
...
//      }
    ...
    BootstrapTable.prototype.load = function (data) {
        ...
        if (this.options.sidePagination === 'server') {
//        		debugger;
            this.options.totalRows = data[this.options.totalName];
            fixedScroll = data.fixedScroll;
            data = data[[this.options.dataName]];
